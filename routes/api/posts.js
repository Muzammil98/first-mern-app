  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id }).then(profile =>
      Post.findById(req.params.id).then(post => {
        // Check for post owner for deletion
...
// @desc    Create comment on post with id
// @access  Private
router.post(
  "/comment/:id",
  passport.authenticate("jwt", { session: false }),
...
// @desc    Delete comment from post with id
// @access  Private
router.delete(
    ...
    Post.findById(req.params.id)
      .then(post => {
        ...
        if (
          ...
          return res.status(404).json({ nocomments: "Comment not found" });
        }
        // Get remove index
